...
//        scope.launch {
        try {
            mPage = 1
            val response = apiService.fetchPosts("asc", mPage, PAGE_SIZE).execute()
            when {
...
//        scope.launch {
        try {
            val response = apiService.fetchPosts("asc", params.key, PAGE_SIZE).execute()
            when {
                response.isSuccessful -> {
                    val listing = response.body()?.items
                    userItemDao.insertList(listing ?: listOf())
...
//        scope.launch {
        try {
            val response =
                apiService.fetchPosts("asc", params.key, PAGE_SIZE).execute()
            when {
                response.isSuccessful -> {
